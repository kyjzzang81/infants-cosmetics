...
// Base64로 암호화된 패스워드 (원본: 8092#$%bambit@#111$%)
const ENCRYPTED_PASSWORD = "ODA5MiMkJWJhbWJpdEAjMTExJCU=";
...
function checkPassword() {
  ...
  // 입력된 패스워드를 Base64로 인코딩해서 비교
  const encryptedInput = btoa(input);

  if (encryptedInput === ENCRYPTED_PASSWORD) {
    isAuthenticated = true;
    statusDiv.innerHTML =
      '<div class="password-success">✅ 인증 성공! 전체 제품을 볼 수 있습니다.</div>';
    ...
    statusDiv.innerHTML =
      '<div class="password-error">❌ 잘못된 패스워드입니다.</div>';
...
function filterProducts() {
  ...
  filteredProducts = products.filter((product) => {
    ...
    const matchesSearch =
      !searchTerm ||
      (product["제품명_정제"] &&
        product["제품명_정제"].toLowerCase().includes(searchTerm)) ||
      (product["브랜드명_kor"] &&
        product["브랜드명_kor"].toLowerCase().includes(searchTerm));
